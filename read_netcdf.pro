...
pro read_netCDF, filename, data, attributes, status
  ...
  if ( finq.ndims gt 0 ) then begin
    ...
    for k=0L,finq.ndims-1 do begin
  ...
  ;
  for k=0L, finq.nvars-1 do begin
...
for k=0L, finq.nvars-1 do begin
  ...
  ;    if (size(value))[1] eq (size(data.(var_inq[k].nest_cnt[0])))[2] then begin
...
for k=0L, finq.nvars-1 do if (var_inq[k].natts gt 0) then num_att = num_att + var_inq[k].natts + 1
